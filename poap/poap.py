...
user_scripts = ['NotifyNSOdeviceOnline.py']
nso_ip = "10.95.5.60"
nso_port = 8080
nso_username = "admin"
nso_password = "admin"
...
def download_scripts_and_agents():
    ...
    destination_location = "/bootflash"

    for user_script in user_scripts:
        remove_file(os.path.join(destination_location, user_script))
        download_user_app(source_location, user_script)
...
def remove_file(filename):
    ...
    """
    poap_log("Removing file: "+filename)
    if filename.endswith('nxos.7.0.3.I6.1.bin'):
        poap_log("Cancelling removal of file: "+filename)
        return True
...
def do_copy(source="", dest="", login_timeout=10, dest_tmp=""):
    ...
    dest_check = os.path.join(options["destination_path"],dest)
    if os.path.exists(dest_check):
        poap_log(dest_check + " Already exists, cancelling download")
        return False
    else:
        poap_log(dest_check + " does not exist, proceeding with download")
...
def nso_request(request_path, operation='GET', payload=''):
    ...
    poap_log("NSO " + operation + "request: " + url)
    ...
    poap_log("NSO request status: " + str(response.status) + " : " + response.reason)
    if operation == 'GET' and response.status != 404:
...
def update_poap_entry_in_nso(serial_number, state, mgmt_ip_address):
    request_path = "/api/running/poap"
    payload = """
<poap>
  <device>
    <id>{SERIAL_NUMBER}</id>
    <poap-state>{STATE}</poap-state>
    <management-ip-address>{MGMT_IP_ADDRESS}</management-ip-address>
  </device>
</poap>
    """.format(SERIAL_NUMBER=serial_number, STATE=state, MGMT_IP_ADDRESS=mgmt_ip_address)
    return nso_request(request_path, 'PATCH', payload)

def create_poap_entry_in_nso(serial_number, mgmt_ip_address):
    request_path = "/api/running/poap"
    payload = """
<device>
  <id>{SERIAL_NUMBER}</id>
  <poap-state>UNCLAIMED</poap-state>
  <management-ip-address>{MGMT_IP_ADDRESS}</management-ip-address>
</device>
    """.format(SERIAL_NUMBER=serial_number, MGMT_IP_ADDRESS=mgmt_ip_address)
    return nso_request(request_path, 'POST', payload)

def retrieve_device_poap_settings(serial_number, mgmt_ip_address):
    request_path = "/api/running/poap/device/"+serial_number
    response = nso_request(request_path)
    if response['status'] == 404:
        poap_log("No device information found in NSO, creating POAP entry in NSO with status UNCLAIMED")
        create_poap_entry_in_nso(serial_number, mgmt_ip_address)
        poap_log("Unable to continue: Please set the device POAP configuration file in NSO" )
        exit(1)
    else:
        response_data = response['data']
        poap_log("Current NSO device POAP status: " + response_data['poap:device']['poap-state'])
        try:
            configuration_file = response_data['poap:device']['configuration-file']
            poap_log("Setting device configuration file to: " + configuration_file)
            options["source_config_file"] = configuration_file
            image_file = response_data['poap:device']['image-file']
            poap_log("Setting device image file to: " + image_file)
            options["target_system_image"] = image_file
            poap_log("Setting NSO device status to : " + 'PROVISIONING')
            update_poap_entry_in_nso(serial_number, response_data['poap:device']['poap-state'], mgmt_ip_address)
            update_device_state_in_nso(serial_number, 'PROVISIONING')
        except KeyError:
            poap_log("Unable to continue: Please set the device POAP configuration file and software image file in NSO" )
            exit(1)
    #raise Exception ('TESTING - ALWAYS FAIL')

def register_device_with_nso():
    poap_log("Registering device with NSO")
    nso_ip = "10.95.5.60"
    nso_port = 8080
    nso_username = "admin"
    nso_password = "admin"
    ...
    poap_log("Device Serial Number: " + serial_number)
    mgmt_ip_address = cli("show running-config interface mgmt 0 | grep 'ip address' | awk '{print $3}' | cut -d '/' -f 1").strip()
    poap_log("Device Mgmt IP Address: " + mgmt_ip_address)
    request_path = "/api/running/devices"
    url = "http://" + nso_ip + ":" + str(nso_port) + request_path
    payload = """
<device>
  <name>{SERIAL_NUMBER}</name>
  <address>{MGMT_IP_ADDRESS}</address>
  ...
  <location>
    ...
    response = conn.getresponse()
    poap_log("NSO device registration status: " + str(response.status) + ":" + response.reason)
    poap_log("NSO device registration complete")

def main():
    poap_log("Running POAP script version "+script_version)
    ...
    mgmt_ip_address = cli("show running-config interface mgmt 0 | grep 'ip address' | awk '{print $3}' | cut -d '/' -f 1").strip()
    poap_log("Device Mgmt IP Address: " + mgmt_ip_address)
    retrieve_device_poap_settings(serial_number, mgmt_ip_address)
    ...
    if options["target_system_image"] != '':
...
#
    ...
    if options["target_system_image"] != '':
        # Copy the software image to the switch
        copy_system()
    ...
    #Johan added
    update_device_state_in_nso(serial_number, 'PROVISIONED')
#    register_device_with_nso()
